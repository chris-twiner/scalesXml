...
Although the base Monad types have a consistent API actually running them is inconsistent.  Id doesn't actually get run, Trampoline/Free has a run function and IO has an unsafePerformIO.
...
val p = doSomethingReturningIdOfA

val a: A = p runIt
```

Won't work for Id as it'll be seen an attempt to call apply by scalac, this will, however, work fine:

```scala
import scales.utils.iteratee.monadHelpers._

val p = doSomethingReturningIdOrTrampolineOrIOOfA

val a: A = p.runIt
```

Morever, map/flatMap'ing Id[IterateeT[E,F,A]] "unpacks" the iteratee forcing you to swap <-'s to = in for comprehensions to keep the IterateeT (otherwise IterateeT.map/flatMap is called, which is likely not desired).

Importing scalaz.Scalaz._ will work for the last generator only, due to this IdHelper is also provided, this should not be used in conjunction with Scalaz._ as BindOps conversions will be triggered.
